...
static const char *const TAG = "Yeetc";


void yeetcComponent::set_pm_1_0_sensor(sensor::Sensor *pm_1_0_sensor) { pm_1_0_sensor_ = pm_1_0_sensor; }
void yeetcComponent::set_pm_2_5_sensor(sensor::Sensor *pm_2_5_sensor) { pm_2_5_sensor_ = pm_2_5_sensor; }
void yeetcComponent::set_pm_10_0_sensor(sensor::Sensor *pm_10_0_sensor) { pm_10_0_sensor_ = pm_10_0_sensor; }

void yeetcComponent::set_pm_particles_03um_sensor(sensor::Sensor *pm_particles_03um_sensor) {
  pm_particles_03um_sensor_ = pm_particles_03um_sensor;
}
void yeetcComponent::set_pm_particles_05um_sensor(sensor::Sensor *pm_particles_05um_sensor) {
  pm_particles_05um_sensor_ = pm_particles_05um_sensor;
}
void yeetcComponent::set_pm_particles_10um_sensor(sensor::Sensor *pm_particles_10um_sensor) {
  pm_particles_10um_sensor_ = pm_particles_10um_sensor;
}
void yeetcComponent::set_pm_particles_25um_sensor(sensor::Sensor *pm_particles_25um_sensor) {
  pm_particles_25um_sensor_ = pm_particles_25um_sensor;
}
void yeetcComponent::set_pm_particles_50um_sensor(sensor::Sensor *pm_particles_50um_sensor) {
  pm_particles_50um_sensor_ = pm_particles_50um_sensor;
}
void yeetcComponent::set_pm_particles_100um_sensor(sensor::Sensor *pm_particles_100um_sensor) {
  pm_particles_100um_sensor_ = pm_particles_100um_sensor;
}

void yeetcComponent::set_temperature_sensor(sensor::Sensor *temperature_sensor) {
  temperature_sensor_ = temperature_sensor;
}
void yeetcComponent::set_humidity_sensor(sensor::Sensor *humidity_sensor) { humidity_sensor_ = humidity_sensor; }
void yeetcComponent::set_formaldehyde_sensor(sensor::Sensor *formaldehyde_sensor) {
  formaldehyde_sensor_ = formaldehyde_sensor;
}

void yeetcComponent::loop() {
  ...
  if (this->update_interval_ > PMS_STABILISING_MS) {
    ...
    switch (this->state_) {
      case yeetc_STATE_IDLE:
        ...
        this->state_ = yeetc_STATE_STABILISING;
        this->send_command_(PMS_CMD_ON_STANDBY, 1);
        this->fan_on_time_ = now;
        return;
      case yeetc_STATE_STABILISING:
        ...
        this->state_ = yeetc_STATE_WAITING;
        break;
      case yeetc_STATE_WAITING:
...
float yeetcComponent::get_setup_priority() const { return setup_priority::DATA; }
optional<bool> yeetcComponent::check_byte_() {
  ...
  if (index == 3) {
    ...
    switch (this->type_) {
      case yeetc_TYPE_AC4G:
        length_matches = payload_length == 28 || payload_length == 20;
        break;
      case yeetc_TYPE_5003T:
      case yeetc_TYPE_5003S:
        length_matches = payload_length == 28;
        break;
      case yeetc_TYPE_5003ST:
        length_matches = payload_length == 36;
        break;
    }

    if (!length_matches) {
      ESP_LOGW(TAG, "yeetc length %u doesn't match. Are you using the correct yeetc type?", payload_length);
      return false;
    }
    return true;
  }
  ...
  if (checksum != check) {
    ESP_LOGW(TAG, "yeetc checksum mismatch! 0x%02X!=0x%02X", checksum, check);
    return false;
  }
...
void yeetcComponent::send_command_(uint8_t cmd, uint16_t data) {
  this->data_index_ = 0;
...
void yeetcComponent::parse_data_() {
  switch (this->type_) {
    case yeetc_TYPE_5003ST: {
    ...
    case yeetc_TYPE_AC4G: {
      ...
      uint16_t pm_particles_03um = this->get_16_bit_uint_(10);
      uint16_t pm_particles_05um = this->get_16_bit_uint_(12);
      uint16_t pm_particles_10um = this->get_16_bit_uint_(14);

      uint16_t pm_particles_25um = this->get_16_bit_uint_(16);
      uint16_t pm_particles_50um = this->get_16_bit_uint_(18);
      uint16_t pm_particles_100um = this->get_16_bit_uint_(20);
  ...
  if (this->update_interval_ > PMS_STABILISING_MS) {
    ...
    this->state_ = yeetc_STATE_IDLE;
  }

  this->status_clear_warning();
}
uint16_t yeetcComponent::get_16_bit_uint_(uint8_t start_index) {
  return (uint16_t(this->data_[start_index]) << 8) | uint16_t(this->data_[start_index + 1]);
}
void yeetcComponent::dump_config() {
  ESP_LOGCONFIG(TAG, "yeetc:");
